...
def get_post_by_date_and_task_id(dt, task_id):
    return DBSession.query(Post).filter(Post.date == dt, Post.task_id == task_id).first()
...
def update_task_id(fb_post_id, task_id):    
    id_update = DBSession.query(Post).filter(Post.fb_post_id == fb_post_id).first()   
    #if not id_update:
        #dt = get_date_time_from_post(browser, post)  # Assuming get_date_time_from_post returns a list of timestamps
        #if dt:                
            #id_update = DBSession.query(Post).filter(Post.date == dt, Post.task_id == task_id).first()
    
    if id_update:
        id_update.task_id = task_id
        logger.log("Updated task_id to {} for post with fb_post_id {}".format(task_id, fb_post_id))
        DBSession.commit()
    else:
        logger.log("Post not found for fb_post_id {} or by date and task_id".format(fb_post_id))
